...
const initialState: ExplorerState = {
  visibleFiles: [],
  visibleFolders: null,
  currentExplorerRoute: null,
};

export const setCurrentExplorerRoute = createAction<
  PrepareAction<RecurciveFileItem | null>
>("explorer/setCurrentExplorerRoute", (folder: RecurciveFileItem) => {
  return {
    payload: folder,
  };
});
  ...
  if(folder && folder.children && IsArray(folder.children)){
    ...
    if(newFolder.dirId === folder.id){
      ...
      for(let i = 0; i < folder.children.length; i++){
        folder.children[i] = recurciveUpdate(folder.children[i], newFolder);
      }
    }
  }
  return folder
...
export const explorerSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(setVisibleFiles, (state, action) => {
      if(action.payload && {}.toString.call(action.payload) === "[object Array]") {
        ...
        if(!state.currentExplorerRoute) state.currentExplorerRoute = rootFolder;
        if(rootFolder){
          let tempRoot = rootFolder;
          folders.forEach(async (item : RecurciveFileItem) => {
            if(IsEmpty(item.dirId)) {
              tempRoot = {...tempRoot, ...item}
            }else{
