import { INCORRECT_PASSWORD, INTERNAL_ERROR, USER_NOTFOUND_ERROR } from "@/lib/const";
import DBConnect from "@/lib/mongo";
import { LoginSchema } from "@/schemas/validators/auth";
import { AuthorizedRequest } from "@/types/api";
import { cleanString } from "@/util";
import { compareHashedPassword, GenerateAuthCredentials, SetClientAuthCookie } from "@/util/auth";
...
async function handler(req: AuthorizedRequest) {
  try{
    ...
    try{
      const user = await getUserByAuthPayloadOrUserName(req, loginData.username)
      ...
      return NextResponse.json(e, {status: e?.responseCode ? e?.responseCode : 500});
    }
  }catch(e){
